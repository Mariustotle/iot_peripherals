...
class RaspberryPi3(DeviceBase):
    ...
    def validate_i2c_pins(
        ...
        if channel not in VALID_I2C_BUSES:
            errors.append(
                f"  ✖ ERROR: Invalid I2C bus {channel}. Raspberry Pi 3/4 only supports external sensors on I2C bus 1."
            )
        ...
        if sda_pin_details.active_pin_type is not PinType.I2C_SDA:
            errors.append(
                f"  ✖ ERROR: SDA pin configured at [H{sda_position.horizontal_position}/V{sda_position.vertical_position}] "
                f"is {sda_pin_details.active_pin_type}, expected I2C_SDA."
            )

        if scl_pin_details.active_pin_type is not PinType.I2C_SCL:
            errors.append(
                f"  ✖ ERROR: SCL pin configured at [H{scl_position.horizontal_position}/V{scl_position.vertical_position}] "
        ...
        # ------------------------------------------------------------
        # FINAL >> Adapter-level validation (your existing logic)
        # ------------------------------------------------------------
        if len(errors) == 0:
            debug_adapter, errors_adapter = self.adapter.validate_i2c(name, channel, i2c_address)
