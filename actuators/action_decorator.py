...
def action(
    ...
    *,
    ...
    params: list[ActionParam] | None = None
):
...
def derive_params_from_signature(fn: Callable[..., Any]) -> list[ActionParam]:
    ...
    for name, p in sig.parameters.items():
        ...
        if isinstance(ann, type) and issubclass(ann, Enum):
            choices = list(ann)
        params.append(ActionParam(name=name, type_=ann, required=required, default=default, choices=choices))
    return params
